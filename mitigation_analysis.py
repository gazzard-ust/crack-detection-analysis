...
def generate_comparison_figure(data, split=SPLIT, output_path=FIGURE_PATH):
    ...
    fig, axes = plt.subplots(1, 2, figsize=(14, 5.5), gridspec_kw={"width_ratios": [3, 1.2]})
    ...
    for i, strategy in enumerate(strategies_present):
        ...
        # Value labels just below the error bar whisker
        for bar, mean, std in zip(bars, means, stds):
            ax.text(
                bar.get_x() + bar.get_width() / 2, mean + std + 0.005,
    ...
    for bar, val in zip(bars, gap_means):
        ax2.text(
            ...
            f"{val:.1f} pp", ha="right", va="center", fontsize=9,
